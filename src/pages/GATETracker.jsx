...
export default function GATETracker() {
  ...
  useEffect(() => {
    if (!myId) return;
    onValue(ref(db, `gate/${myId}/subjects`), snap => setMyProgress(snap.val() || {}));
    onValue(ref(db, `gate/${myId}/targets`), snap => {
      const data = snap.val();
      if (data) setMyTargets(Object.entries(data).map(([id, v]) => ({ id, ...v })));
      else setMyTargets([]);
    });
    onValue(ref(db, "gate"), snap => {
      const data = snap.val();
      if (!data) return;
      const otherEntry = Object.entries(data).find(([uid]) => uid !== myId);
      if (otherEntry) {
        setOtherProgress(otherEntry[1].subjects || {});
        const t = otherEntry[1].targets;
  ...
  function updateSubject(subject, delta) {
    const current = myProgress[subject] || 0;
    const next = Math.max(0, current + delta);
    set(ref(db, `gate/${myId}/subjects/${subject}`), next);
  }

  function addTarget() {
    if (!newTarget.trim()) return;
    push(ref(db, `gate/${myId}/targets`), { text: newTarget, done: false });
    setNewTarget("");
  }

  function toggleTarget(id, done) {
    set(ref(db, `gate/${myId}/targets/${id}/done`), !done);
  }

  function deleteTarget(id) {
    remove(ref(db, `gate/${myId}/targets/${id}`));
  }
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
        ...
        <div style={{ ...card, opacity: 0.85 }}>
          ...
          <p style={{ fontSize: "11px", color: "#555", marginBottom: "12px" }}>
            {otherTargets.filter(t => t.done).length}/{otherTargets.length} done
          </p>
